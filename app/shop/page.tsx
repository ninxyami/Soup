...
export default function ShopPage() {
  ...
  const displayed = items
    ...
    .filter((i) => tab === "sell" ? i.sell_price != null : true);

  const buyItem = async (item: ShopItem) => {
    if (!user) { window.location.href = `${API}/auth/discord/login?redirect=shop`; return; }
    if (!confirm(`Buy ${item.name} for ${item.buy_price} 🟤?`)) return;
  ...
  return (
    <div className="scanline min-h-screen" style={{ background: "#080a0c", color: "#c8cdd6", fontFamily: "'Inter', sans-serif" }}>
      ...
      <header className="flex items-center gap-4 px-8 py-4 border-b border-[#1e2530] bg-[#0f1318] sticky top-[49px] z-10">
        ...
        {user ? (
          <div className="flex items-center gap-3">
            ...
            <button onClick={async () => { await fetch(`${API}/auth/logout`, { method: "POST", credentials: "include" }); window.location.reload(); }}
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-8" style={{ display: "grid", gridTemplateColumns: "180px 1fr", gap: "2rem" }}>
        ...
        {/* Main */}
        <div>
          <div className="flex border-b border-[#1e2530] mb-6">
            {(["buy", "sell"] as const).map((t) => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-6 py-2 font-mono text-xs tracking-[2px] uppercase border-b-2 -mb-px cursor-pointer bg-transparent border-t-0 border-x-0 transition-all ${
                  tab === t ? "border-accent text-accent" : "border-transparent text-dim hover:text-[#c8cdd6]"
                }`}>
                {t === "buy" ? "🛒 Buy" : "💰 Sell"}
              </button>
            ))}
          ...
          <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))" }}>
            {displayed.map((item) => {
              ...
              return (
                <div key={item.item_id} className={`bg-[#0f1318] border border-[#1e2530] p-4 relative transition-all hover:border-[rgba(200,168,75,0.3)] hover:-translate-y-[2px] ${outOfStock && tab === "buy" ? "opacity-50" : ""}`}>
                  ...
                  <div className="font-mono text-lg text-accent mb-1">{price != null ? `${price} 🟤` : "—"}</div>
                  <div className="font-mono text-xs mb-3" style={{ color: stockColor }}>{stockText}</div>
                  <button
                    onClick={() => tab === "buy" ? buyItem(item) : showToast("Sell from website is not supported — use in-game shop.", "info")}
                    disabled={outOfStock && tab === "buy"}
                    className={`w-full py-2 font-mono text-xs uppercase tracking-widest border cursor-pointer transition-all disabled:opacity-40 disabled:cursor-not-allowed ${
                      tab === "buy" ? "border-success text-success hover:bg-success hover:text-black" : "border-accent text-accent hover:bg-accent hover:text-black"
                    }`}>
                    {outOfStock && tab === "buy" ? "Out of Stock" : tab === "buy" ? (user ? "Buy" : "🔑 Login") : `Sell for ${item.sell_price} 🟤`}
                  </button>
                </div>
              );
            })}
            ...
            <div className="border border-[#1e2530] bg-[#0f1318] p-6 mt-8">
              ...
              <div className="flex gap-3 items-end flex-wrap">
                <div className="flex flex-col gap-1 flex-1">
                  ...
                  <input value={transferTo} onChange={(e) => setTransferTo(e.target.value)} placeholder="PlayerName" className="bg-[#080a0c] border border-[#1e2530] text-[#c8cdd6] px-3 py-2 font-mono text-sm outline-none focus:border-accent" />
                </div>
                <div className="flex flex-col gap-1" style={{ minWidth: 120 }}>
                  ...
                  <input type="number" value={transferAmt} onChange={(e) => setTransferAmt(e.target.value)} placeholder="100" className="bg-[#080a0c] border border-[#1e2530] text-[#c8cdd6] px-3 py-2 font-mono text-sm outline-none focus:border-accent" />
                </div>
                <div className="flex flex-col gap-1 flex-[1.5]">
                  ...
                  <input value={transferNote} onChange={(e) => setTransferNote(e.target.value)} placeholder="thanks for the save!" className="bg-[#080a0c] border border-[#1e2530] text-[#c8cdd6] px-3 py-2 font-mono text-sm outline-none focus:border-accent" />
                </div>
                <button onClick={doTransfer} className="px-5 py-2 border border-info text-info font-mono text-xs uppercase tracking-widest hover:bg-info hover:text-white transition-all whitespace-nowrap cursor-pointer bg-transparent">
                  Send →
                </button>
              </div>
            </div>
