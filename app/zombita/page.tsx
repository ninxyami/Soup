...
export default function ZombitaPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ fontFamily: "'Inter', sans-serif" }}>
      <div className="flex-1 max-w-[1100px] mx-auto w-full px-4 sm:px-6 py-6 sm:py-12">

        {/* Mobile: avatar toggle button */}
        <div className="sm:hidden flex items-center gap-3 mb-4">
          <button
            onClick={() => setShowAvatar(v => !v)}
            className="flex items-center gap-2 bg-transparent border border-[#222] px-3 py-1.5 text-[0.72rem] font-mono text-[#555] hover:text-[#e6e6e6] cursor-pointer"
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={avatar} alt="Zombita" className="w-6 h-6 rounded-full object-cover" />
            ZOMBITA · {mood}
          </button>
          <span className="text-[0.68rem] text-[#333] font-mono">{subtitle}</span>
        </div>

        {/* Mobile: collapsible avatar */}
        {showAvatar && (
          <div className="sm:hidden flex flex-col items-center mb-6">
            <div className="w-[200px] h-[200px] animate-breathe">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={avatar} alt="Zombita" className="w-full h-full object-contain" style={{ filter: "drop-shadow(0 8px 32px rgba(0,0,0,0.6))" }} />
            </div>
          </div>
        )}

        {/* Desktop: side-by-side layout */}
        <div className="hidden sm:grid gap-8 items-start" style={{ gridTemplateColumns: "380px 1fr" }}>
          {/* Avatar panel */}
          <div className="sticky top-20 flex flex-col items-center gap-0 select-none">
            <div className="relative w-[340px] h-[340px]">
              <div className="absolute inset-0 flex items-center justify-center animate-breathe">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={avatar} alt="Zombita" className="w-full h-full object-contain transition-all duration-500" style={{ filter: "drop-shadow(0 8px 32px rgba(0,0,0,0.6))" }} />
              </div>
            </div>
            <div className="text-center mt-2">
              <div className="font-display text-5xl tracking-[0.15em] text-[#e6e6e6]" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>ZOMBITA</div>
...
function ChatPanel({ scrollRef, inputRef, msgs, loading, locked, isAuthed, remaining, input, setInput, send }: any) {
  return (
    <div className="flex flex-col" style={{ height: "calc(100vh - 200px)", minHeight: 400 }}>
      <div ref={scrollRef} className="flex-1 overflow-y-auto flex flex-col gap-3 pr-1 pb-4" style={{ scrollbarWidth: "thin", scrollbarColor: "#2a2a2a transparent" }}>
        ...
        {msgs.map((m: any, i: number) => (
          <div key={i} className={`flex flex-col max-w-[85%] ${m.role === "user" ? "self-end" : "self-start"}`}>
            ...
            <div className={`px-3 py-2 sm:px-4 sm:py-3 text-[0.85rem] sm:text-[0.9rem] leading-relaxed ${
              m.role === "user" ? "bg-[#141414] border border-[#2a2a2a] text-[#e6e6e6]" : "bg-transparent border border-[#1a1a1a] text-[#c8cdd6]"
            }`}>
              {m.text}
            </div>
          </div>
      ...
      {locked && (
        <div className="py-6 text-center border border-[#1a1a1a] mb-2">
      ...
      {!locked && (
        <div className="flex gap-2 sm:gap-3 items-end border-t border-[#1a1a1a] pt-3 sm:pt-4">
          <textarea
            ref={inputRef}
            value={input}
            onChange={(e) => {
            ...
            className="flex-1 bg-transparent border border-[#1a1a1a] text-[#e6e6e6] text-[0.85rem] sm:text-[0.9rem] px-3 py-2 sm:px-4 sm:py-3 outline-none focus:border-[#4a7c59] font-mono placeholder:text-[#333] transition-colors resize-none overflow-hidden"
            style={{ fontFamily: "inherit" }}
          />
          <button
            onClick={send}
            disabled={loading || !input.trim()}
            className="px-4 py-2 sm:px-5 sm:py-3 border border-[#4a7c59] text-[#4a7c59] font-mono text-[0.8rem] uppercase tracking-widest hover:bg-[#4a7c59] hover:text-black disabled:opacity-30 disabled:cursor-not-allowed transition-all cursor-pointer bg-transparent flex-shrink-0"
          >
            →
          </button>
        </div>
