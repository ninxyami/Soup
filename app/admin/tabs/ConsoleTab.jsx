...
const SOURCES = [
  ...
  { key: "pz",   label: "🧟 PZ Server",    desc: "debug log"         },
];
...
const MAX_LINES = 500;   // cap in-memory lines to avoid memory bloat
...
export default function ConsoleTab() {
  ...
  const switchSource = useCallback((src) => {
    setActiveSource(src);
    setLines([]);   // clear display, fresh backlog coming from server
  ...
  return (
    <div>
      <Title t="🖥️ Server Console" s="Live log viewer and RCON terminal" />
          ...
          {visibleLines.map(l => (
            <div
              ...
              style={{
                ...
                paddingLeft:   l.color === "err" || l.color === "warn" || l.color === "cmd" ? 8 : 0,
                marginBottom:  1,
                wordBreak:     "break-all",
                whiteSpace:    "pre-wrap",
              }}
      ...
      <div className="ap-fb" style={{ marginTop: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <B
            ...
            sm
            onClick={sendCmd}
            disabled={status !== "connected" || !cmd.trim()}
          >
            Send ↵
          </B>
        </div>
